...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def init(self):
        self.db = mysql.connector.connect(host='13.209.3.181', user='aelim', password='1234', database='aelimDB', auth_plugin='mysql_native_password')
    ...
    def pollingQuery(self):
        ...
        self.cur.execute("select * from info_car limit 15")
        self.ui.sensingText.clear()
        for (speed,acceleration) in self.cur:
            str = "%d |%f" % (speed,acceleration)
            self.ui.sensingText.appendPlainText(str)
        self.db.commit()
